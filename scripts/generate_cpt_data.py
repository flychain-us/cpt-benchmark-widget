#!/usr/bin/env python3
"""
Generate CPT Benchmark Data from CMS Medicare Fee Schedule

This script parses CMS data files and generates a JavaScript file containing:
- CPT codes with descriptions
- State-by-state Medicare payment averages
- Geographic adjustments based on GPCI

Data Sources:
- PPRRVU2025_Oct.csv: CPT codes, RVUs, and descriptions
- GPCI2025.csv: Geographic Practice Cost Indices by state/locality

Medicare Payment Formula:
Payment = [(Work RVU × Work GPCI) + (PE RVU × PE GPCI) + (MP RVU × MP GPCI)] × Conversion Factor
"""

import csv
import json
from collections import defaultdict
from pathlib import Path

# 2025 Medicare Conversion Factor
CONVERSION_FACTOR = 32.3465

def parse_gpci_data(filepath):
    """
    Parse GPCI (Geographic Practice Cost Index) data.
    Returns a dict: state -> list of localities with GPCI values
    """
    gpci_by_state = defaultdict(list)
    
    with open(filepath, 'r', encoding='utf-8') as f:
        reader = csv.reader(f)
        # Skip header rows
        for _ in range(3):
            next(reader, None)
        
        for row in reader:
            if len(row) < 7 or not row[1].strip():
                continue
            
            try:
                state = row[1].strip()
                locality = row[3].strip()
                pw_gpci = float(row[4]) if row[4] else 1.0  # Work GPCI
                pe_gpci = float(row[5]) if row[5] else 1.0  # Practice Expense GPCI
                mp_gpci = float(row[6]) if row[6] else 1.0  # Malpractice GPCI
                
                gpci_by_state[state].append({
                    'locality': locality,
                    'pw_gpci': pw_gpci,
                    'pe_gpci': pe_gpci,
                    'mp_gpci': mp_gpci
                })
            except (ValueError, IndexError):
                continue
    
    return gpci_by_state

def get_state_average_gpci(gpci_by_state):
    """
    Calculate the average GPCI for each state (averaging across localities).
    """
    state_averages = {}
    
    for state, localities in gpci_by_state.items():
        if not localities:
            continue
        
        avg_pw = sum(loc['pw_gpci'] for loc in localities) / len(localities)
        avg_pe = sum(loc['pe_gpci'] for loc in localities) / len(localities)
        avg_mp = sum(loc['mp_gpci'] for loc in localities) / len(localities)
        
        state_averages[state] = {
            'pw_gpci': avg_pw,
            'pe_gpci': avg_pe,
            'mp_gpci': avg_mp
        }
    
    return state_averages

def parse_rvu_data(filepath):
    """
    Parse RVU (Relative Value Unit) data from the PPRRVU file.
    Returns a dict: HCPCS (CPT code) -> RVU data
    """
    cpt_data = {}
    
    with open(filepath, 'r', encoding='utf-8') as f:
        reader = csv.reader(f)
        # Skip header rows (first 8 lines are headers/copyright)
        for _ in range(8):
            next(reader, None)
        
        for row in reader:
            if len(row) < 12:
                continue
            
            hcpcs = row[0].strip()
            if not hcpcs or not hcpcs[0].isdigit():
                continue
            
            # Skip codes with status indicating not payable
            status = row[3].strip()
            if status in ['I', 'B', 'D', 'M', 'N', 'E', 'X']:
                continue
            
            try:
                description = row[2].strip()
                work_rvu = float(row[5]) if row[5] else 0.0
                nf_pe_rvu = float(row[6]) if row[6] else 0.0  # Non-facility PE RVU
                fac_pe_rvu = float(row[8]) if row[8] else 0.0  # Facility PE RVU
                mp_rvu = float(row[10]) if row[10] else 0.0  # Malpractice RVU
                
                # Use non-facility PE RVU (more common for outpatient/office settings)
                pe_rvu = nf_pe_rvu if nf_pe_rvu > 0 else fac_pe_rvu
                
                # Skip codes with zero total RVU
                if work_rvu == 0 and pe_rvu == 0 and mp_rvu == 0:
                    continue
                
                cpt_data[hcpcs] = {
                    'description': description,
                    'work_rvu': work_rvu,
                    'pe_rvu': pe_rvu,
                    'mp_rvu': mp_rvu
                }
            except (ValueError, IndexError):
                continue
    
    return cpt_data

def calculate_payment(work_rvu, pe_rvu, mp_rvu, gpci, cf=CONVERSION_FACTOR):
    """
    Calculate Medicare payment amount using the standard formula.
    """
    payment = (
        (work_rvu * gpci['pw_gpci']) +
        (pe_rvu * gpci['pe_gpci']) +
        (mp_rvu * gpci['mp_gpci'])
    ) * cf
    return round(payment, 2)

def generate_state_averages(cpt_data, state_gpcis):
    """
    Generate state-by-state payment averages for each CPT code.
    """
    result = {}
    
    for hcpcs, rvu_data in cpt_data.items():
        state_payments = {}
        
        for state, gpci in state_gpcis.items():
            payment = calculate_payment(
                rvu_data['work_rvu'],
                rvu_data['pe_rvu'],
                rvu_data['mp_rvu'],
                gpci
            )
            if payment > 0:
                state_payments[state] = payment
        
        if state_payments:
            result[hcpcs] = {
                'description': rvu_data['description'],
                'stateAverages': state_payments
            }
    
    return result

# Human-readable CPT descriptions (CMS uses abbreviated names)
CPT_DESCRIPTIONS = {
    # Office/Outpatient E&M - New Patient
    '99201': 'New Patient Office Visit (10 min)',
    '99202': 'New Patient Office Visit (20 min)',
    '99203': 'New Patient Office Visit (30 min)',
    '99204': 'New Patient Office Visit (45 min)',
    '99205': 'New Patient Office Visit (60 min)',
    
    # Office/Outpatient E&M - Established Patient
    '99211': 'Established Patient Office Visit (5 min)',
    '99212': 'Established Patient Office Visit (10 min)',
    '99213': 'Established Patient Office Visit (20 min)',
    '99214': 'Established Patient Office Visit (30 min)',
    '99215': 'Established Patient Office Visit (40 min)',
    
    # Initial Hospital/Observation
    '99221': 'Initial Hospital/Observation - Low Complexity (40 min)',
    '99222': 'Initial Hospital/Observation - Moderate Complexity (55 min)',
    '99223': 'Initial Hospital/Observation - High Complexity (75 min)',
    
    # Subsequent Hospital Care
    '99231': 'Subsequent Hospital Care - Low Complexity (25 min)',
    '99232': 'Subsequent Hospital Care - Moderate Complexity (35 min)',
    '99233': 'Subsequent Hospital Care - High Complexity (50 min)',
    
    # Hospital Discharge
    '99238': 'Hospital Discharge Day (30 min or less)',
    '99239': 'Hospital Discharge Day (more than 30 min)',
    
    # Preventive Medicine - New Patient
    '99381': 'Preventive Visit - New Patient, Infant (under 1 year)',
    '99382': 'Preventive Visit - New Patient, Age 1-4',
    '99383': 'Preventive Visit - New Patient, Age 5-11',
    '99384': 'Preventive Visit - New Patient, Age 12-17',
    '99385': 'Preventive Visit - New Patient, Age 18-39',
    '99386': 'Preventive Visit - New Patient, Age 40-64',
    '99387': 'Preventive Visit - New Patient, Age 65+',
    
    # Preventive Medicine - Established Patient
    '99391': 'Preventive Visit - Established Patient, Infant (under 1 year)',
    '99392': 'Preventive Visit - Established Patient, Age 1-4',
    '99393': 'Preventive Visit - Established Patient, Age 5-11',
    '99394': 'Preventive Visit - Established Patient, Age 12-17',
    '99395': 'Preventive Visit - Established Patient, Age 18-39',
    '99396': 'Preventive Visit - Established Patient, Age 40-64',
    '99397': 'Preventive Visit - Established Patient, Age 65+',
    
    # Consultation Codes
    '99241': 'Office Consultation - Brief (15 min)',
    '99242': 'Office Consultation - Limited (30 min)',
    '99243': 'Office Consultation - Intermediate (40 min)',
    '99244': 'Office Consultation - Moderate (60 min)',
    '99245': 'Office Consultation - Comprehensive (80 min)',
    
    # Emergency Department
    '99281': 'Emergency Department Visit - Minimal',
    '99282': 'Emergency Department Visit - Low to Moderate',
    '99283': 'Emergency Department Visit - Moderate',
    '99284': 'Emergency Department Visit - High',
    '99285': 'Emergency Department Visit - Highest Severity',
    
    # Critical Care
    '99291': 'Critical Care - First 30-74 min',
    '99292': 'Critical Care - Each Additional 30 min',
    
    # Telehealth/Phone Visits
    '99441': 'Phone E/M Service (5-10 min)',
    '99442': 'Phone E/M Service (11-20 min)',
    '99443': 'Phone E/M Service (21-30 min)',
    
    # Online Digital E/M
    '99421': 'Online Digital E/M (5-10 min)',
    '99422': 'Online Digital E/M (11-20 min)',
    '99423': 'Online Digital E/M (21+ min)',
    
    # Physical Therapy
    '97110': 'Therapeutic Exercises (15 min)',
    '97112': 'Neuromuscular Re-education (15 min)',
    '97116': 'Gait Training/Therapy (15 min)',
    '97140': 'Manual Therapy Techniques (15 min)',
    '97530': 'Therapeutic Activities (15 min)',
    '97535': 'Self-Care/Home Management Training (15 min)',
    '97542': 'Wheelchair Management Training (15 min)',
    '97150': 'Group Therapeutic Procedures',
    '97161': 'PT Evaluation - Low Complexity',
    '97162': 'PT Evaluation - Moderate Complexity',
    '97163': 'PT Evaluation - High Complexity',
    '97164': 'PT Re-evaluation',
    
    # Psychotherapy
    '90832': 'Psychotherapy (16-37 min)',
    '90834': 'Psychotherapy (38-52 min)',
    '90837': 'Psychotherapy (53+ min)',
    '90839': 'Crisis Psychotherapy - First 60 min',
    '90840': 'Crisis Psychotherapy - Each Additional 30 min',
    '90846': 'Family Psychotherapy (without patient)',
    '90847': 'Family Psychotherapy (with patient)',
    '90853': 'Group Psychotherapy',
    
    # Psychiatric Diagnostic
    '90791': 'Psychiatric Diagnostic Evaluation',
    '90792': 'Psychiatric Diagnostic Evaluation with Medical Services',
    
    # Psychological/Neuropsychological Testing
    '96130': 'Psychological Testing Evaluation - First Hour',
    '96131': 'Psychological Testing Evaluation - Additional Hour',
    '96136': 'Psychological Test Administration - First 30 min',
    '96137': 'Psychological Test Administration - Additional 30 min',
    '96138': 'Tech-Administered Psych Test - First 30 min',
    '96139': 'Tech-Administered Psych Test - Additional 30 min',
    
    # Chiropractic
    '98940': 'Chiropractic Manipulation - Spinal 1-2 Regions',
    '98941': 'Chiropractic Manipulation - Spinal 3-4 Regions',
    '98942': 'Chiropractic Manipulation - Spinal 5 Regions',
    '98943': 'Chiropractic Manipulation - Extraspinal',
    
    # Acupuncture
    '97810': 'Acupuncture with Manual Stimulation - Initial 15 min',
    '97811': 'Acupuncture with Manual Stimulation - Additional 15 min',
    '97813': 'Acupuncture with Electrical Stimulation - Initial 15 min',
    '97814': 'Acupuncture with Electrical Stimulation - Additional 15 min',
    
    # Nursing Facility
    '99304': 'Nursing Facility - Initial Visit, Low Complexity',
    '99305': 'Nursing Facility - Initial Visit, Moderate Complexity',
    '99306': 'Nursing Facility - Initial Visit, High Complexity',
    '99307': 'Nursing Facility - Subsequent Visit, Straightforward',
    '99308': 'Nursing Facility - Subsequent Visit, Low Complexity',
    '99309': 'Nursing Facility - Subsequent Visit, Moderate Complexity',
    '99310': 'Nursing Facility - Subsequent Visit, High Complexity',
    
    # Home Services
    '99341': 'Home Visit - New Patient, Low Complexity (20 min)',
    '99342': 'Home Visit - New Patient, Moderate Complexity (30 min)',
    '99343': 'Home Visit - New Patient, Moderate/High Complexity (45 min)',
    '99344': 'Home Visit - New Patient, High Complexity (60 min)',
    '99345': 'Home Visit - New Patient, Highest Complexity (75 min)',
    '99347': 'Home Visit - Established Patient, Straightforward (15 min)',
    '99348': 'Home Visit - Established Patient, Low Complexity (25 min)',
    '99349': 'Home Visit - Established Patient, Moderate Complexity (40 min)',
    '99350': 'Home Visit - Established Patient, High Complexity (60 min)',
    
    # Care Management
    '99490': 'Chronic Care Management (20 min/month)',
    '99491': 'Chronic Care Management by Physician (30 min/month)',
    
    # Prolonged Services
    '99354': 'Prolonged Office Service - First Hour',
    '99355': 'Prolonged Office Service - Each Additional 30 min',
    '99356': 'Prolonged Inpatient Service - First Hour',
    '99357': 'Prolonged Inpatient Service - Each Additional 30 min',
}


def generate_javascript(cpt_data, output_path):
    """
    Generate the JavaScript file with CPT data.
    """
    # Get list of commonly used CPT codes (E&M, therapy, psych, etc.)
    # For the initial release, let's include ~100 most relevant codes
    common_prefixes = [
        '992',   # E&M office visits
        '993',   # E&M other
        '971',   # Physical therapy
        '908',   # Psychotherapy
        '964',   # Assessments
        '903',   # Dialysis
        '110',   # Skin procedures
        '360',   # Eye procedures
        '430',   # Cardiovascular
    ]
    
    # Filter to most relevant codes (to avoid bloating the file)
    filtered_data = {}
    count = 0
    max_codes = 500  # Limit for reasonable file size
    
    # First pass: get common E&M codes
    priority_codes = ['99213', '99214', '99215', '99212', '99211', 
                      '99203', '99204', '99205', '99202', '99201',
                      '99381', '99382', '99383', '99384', '99385', '99386', '99387',
                      '99391', '99392', '99393', '99394', '99395', '99396', '99397',
                      '97110', '97112', '97116', '97140', '97530', '97535', '97542',
                      '90837', '90834', '90832', '90847', '90846', '90853',
                      '96130', '96131', '96136', '96137',
                      '99441', '99442', '99443',  # Telehealth
                      '99421', '99422', '99423',  # Online services
                      ]
    
    for code in priority_codes:
        if code in cpt_data:
            filtered_data[code] = cpt_data[code]
            count += 1
    
    # Second pass: add other relevant codes
    for hcpcs, data in cpt_data.items():
        if count >= max_codes:
            break
        if hcpcs in filtered_data:
            continue
        for prefix in common_prefixes:
            if hcpcs.startswith(prefix):
                filtered_data[hcpcs] = data
                count += 1
                break
    
    # Replace abbreviated CMS descriptions with human-readable ones
    for code, data in filtered_data.items():
        if code in CPT_DESCRIPTIONS:
            data['description'] = CPT_DESCRIPTIONS[code]
    
    print(f"Included {len(filtered_data)} CPT codes")
    
    # Generate JavaScript header
    js_output = f'''// CPT Benchmark Data - Generated from CMS Medicare Fee Schedule 2025
// Source: Centers for Medicare & Medicaid Services (CMS)
// Conversion Factor: ${CONVERSION_FACTOR:.4f}
// Last Updated: 2025

'''
    
    # Convert to JSON then format as JS
    js_output += "const CPT_DATA = " + json.dumps(filtered_data, indent=2) + ";\n\n"
    
    # Add the ZIP_TO_STATE mapping (keeping the existing one)
    js_output += '''// Zipcode to State mapping (first 3 digits)
const ZIP_TO_STATE = {
    '005': 'NY', '006': 'PR', '007': 'PR', '008': 'PR', '009': 'PR',
    '010': 'MA', '011': 'MA', '012': 'MA', '013': 'MA', '014': 'MA',
    '015': 'MA', '016': 'MA', '017': 'MA', '018': 'MA', '019': 'MA',
    '020': 'MA', '021': 'MA', '022': 'MA', '023': 'MA', '024': 'MA',
    '025': 'MA', '026': 'MA', '027': 'MA', '028': 'RI', '029': 'RI',
    '030': 'NH', '031': 'NH', '032': 'NH', '033': 'NH', '034': 'NH',
    '035': 'VT', '036': 'VT', '037': 'VT', '038': 'VT', '039': 'ME',
    '040': 'ME', '041': 'ME', '042': 'ME', '043': 'ME', '044': 'ME',
    '045': 'ME', '046': 'ME', '047': 'ME', '048': 'ME', '049': 'ME',
    '050': 'VT', '051': 'VT', '052': 'VT', '053': 'VT', '054': 'VT',
    '055': 'MA', '056': 'VT', '057': 'VT', '058': 'VT', '059': 'VT',
    '060': 'CT', '061': 'CT', '062': 'CT', '063': 'CT', '064': 'CT',
    '065': 'CT', '066': 'CT', '067': 'CT', '068': 'CT', '069': 'CT',
    '070': 'NJ', '071': 'NJ', '072': 'NJ', '073': 'NJ', '074': 'NJ',
    '075': 'NJ', '076': 'NJ', '077': 'NJ', '078': 'NJ', '079': 'NJ',
    '080': 'NJ', '081': 'NJ', '082': 'NJ', '083': 'NJ', '084': 'NJ',
    '085': 'NJ', '086': 'NJ', '087': 'NJ', '088': 'NJ', '089': 'NJ',
    '100': 'NY', '101': 'NY', '102': 'NY', '103': 'NY', '104': 'NY',
    '105': 'NY', '106': 'NY', '107': 'NY', '108': 'NY', '109': 'NY',
    '110': 'NY', '111': 'NY', '112': 'NY', '113': 'NY', '114': 'NY',
    '115': 'NY', '116': 'NY', '117': 'NY', '118': 'NY', '119': 'NY',
    '120': 'NY', '121': 'NY', '122': 'NY', '123': 'NY', '124': 'NY',
    '125': 'NY', '126': 'NY', '127': 'NY', '128': 'NY', '129': 'NY',
    '130': 'NY', '131': 'NY', '132': 'NY', '133': 'NY', '134': 'NY',
    '135': 'NY', '136': 'NY', '137': 'NY', '138': 'NY', '139': 'NY',
    '140': 'NY', '141': 'NY', '142': 'NY', '143': 'NY', '144': 'NY',
    '145': 'NY', '146': 'NY', '147': 'NY', '148': 'NY', '149': 'NY',
    '150': 'PA', '151': 'PA', '152': 'PA', '153': 'PA', '154': 'PA',
    '155': 'PA', '156': 'PA', '157': 'PA', '158': 'PA', '159': 'PA',
    '160': 'PA', '161': 'PA', '162': 'PA', '163': 'PA', '164': 'PA',
    '165': 'PA', '166': 'PA', '167': 'PA', '168': 'PA', '169': 'PA',
    '170': 'PA', '171': 'PA', '172': 'PA', '173': 'PA', '174': 'PA',
    '175': 'PA', '176': 'PA', '177': 'PA', '178': 'PA', '179': 'PA',
    '180': 'PA', '181': 'PA', '182': 'PA', '183': 'PA', '184': 'PA',
    '185': 'PA', '186': 'PA', '187': 'PA', '188': 'PA', '189': 'PA',
    '190': 'PA', '191': 'PA', '192': 'PA', '193': 'PA', '194': 'PA',
    '195': 'PA', '196': 'PA',
    '197': 'DE', '198': 'DE', '199': 'DE',
    '200': 'DC', '201': 'VA', '202': 'DC', '203': 'DC', '204': 'DC',
    '205': 'DC', '206': 'MD', '207': 'MD', '208': 'MD', '209': 'MD',
    '210': 'MD', '211': 'MD', '212': 'MD', '214': 'MD', '215': 'MD',
    '216': 'MD', '217': 'MD', '218': 'MD', '219': 'MD',
    '220': 'VA', '221': 'VA', '222': 'VA', '223': 'VA', '224': 'VA',
    '225': 'VA', '226': 'VA', '227': 'VA', '228': 'VA', '229': 'VA',
    '230': 'VA', '231': 'VA', '232': 'VA', '233': 'VA', '234': 'VA',
    '235': 'VA', '236': 'VA', '237': 'VA', '238': 'VA', '239': 'VA',
    '240': 'VA', '241': 'VA', '242': 'VA', '243': 'VA', '244': 'VA',
    '245': 'VA', '246': 'VA',
    '247': 'WV', '248': 'WV', '249': 'WV', '250': 'WV', '251': 'WV',
    '252': 'WV', '253': 'WV', '254': 'WV', '255': 'WV', '256': 'WV',
    '257': 'WV', '258': 'WV', '259': 'WV', '260': 'WV', '261': 'WV',
    '262': 'WV', '263': 'WV', '264': 'WV', '265': 'WV', '266': 'WV',
    '267': 'WV', '268': 'WV',
    '270': 'NC', '271': 'NC', '272': 'NC', '273': 'NC', '274': 'NC',
    '275': 'NC', '276': 'NC', '277': 'NC', '278': 'NC', '279': 'NC',
    '280': 'NC', '281': 'NC', '282': 'NC', '283': 'NC', '284': 'NC',
    '285': 'NC', '286': 'NC', '287': 'NC', '288': 'NC', '289': 'NC',
    '290': 'SC', '291': 'SC', '292': 'SC', '293': 'SC', '294': 'SC',
    '295': 'SC', '296': 'SC', '297': 'SC', '298': 'SC', '299': 'SC',
    '300': 'GA', '301': 'GA', '302': 'GA', '303': 'GA', '304': 'GA',
    '305': 'GA', '306': 'GA', '307': 'GA', '308': 'GA', '309': 'GA',
    '310': 'GA', '311': 'GA', '312': 'GA', '313': 'GA', '314': 'GA',
    '315': 'GA', '316': 'GA', '317': 'GA', '318': 'GA', '319': 'GA',
    '320': 'FL', '321': 'FL', '322': 'FL', '323': 'FL', '324': 'FL',
    '325': 'FL', '326': 'FL', '327': 'FL', '328': 'FL', '329': 'FL',
    '330': 'FL', '331': 'FL', '332': 'FL', '333': 'FL', '334': 'FL',
    '335': 'FL', '336': 'FL', '337': 'FL', '338': 'FL', '339': 'FL',
    '340': 'FL', '341': 'FL', '342': 'FL', '344': 'FL', '346': 'FL',
    '347': 'FL', '349': 'FL',
    '350': 'AL', '351': 'AL', '352': 'AL', '354': 'AL', '355': 'AL',
    '356': 'AL', '357': 'AL', '358': 'AL', '359': 'AL', '360': 'AL',
    '361': 'AL', '362': 'AL', '363': 'AL', '364': 'AL', '365': 'AL',
    '366': 'AL', '367': 'AL', '368': 'AL', '369': 'AL',
    '370': 'TN', '371': 'TN', '372': 'TN', '373': 'TN', '374': 'TN',
    '375': 'TN', '376': 'TN', '377': 'TN', '378': 'TN', '379': 'TN',
    '380': 'TN', '381': 'TN', '382': 'TN', '383': 'TN', '384': 'TN',
    '385': 'TN',
    '386': 'MS', '387': 'MS', '388': 'MS', '389': 'MS', '390': 'MS',
    '391': 'MS', '392': 'MS', '393': 'MS', '394': 'MS', '395': 'MS',
    '396': 'MS', '397': 'MS',
    '400': 'KY', '401': 'KY', '402': 'KY', '403': 'KY', '404': 'KY',
    '405': 'KY', '406': 'KY', '407': 'KY', '408': 'KY', '409': 'KY',
    '410': 'KY', '411': 'KY', '412': 'KY', '413': 'KY', '414': 'KY',
    '415': 'KY', '416': 'KY', '417': 'KY', '418': 'KY',
    '420': 'KY', '421': 'KY', '422': 'KY', '423': 'KY', '424': 'KY',
    '425': 'KY', '426': 'KY', '427': 'KY',
    '430': 'OH', '431': 'OH', '432': 'OH', '433': 'OH', '434': 'OH',
    '435': 'OH', '436': 'OH', '437': 'OH', '438': 'OH', '439': 'OH',
    '440': 'OH', '441': 'OH', '442': 'OH', '443': 'OH', '444': 'OH',
    '445': 'OH', '446': 'OH', '447': 'OH', '448': 'OH', '449': 'OH',
    '450': 'OH', '451': 'OH', '452': 'OH', '453': 'OH', '454': 'OH',
    '455': 'OH', '456': 'OH', '457': 'OH', '458': 'OH', '459': 'OH',
    '460': 'IN', '461': 'IN', '462': 'IN', '463': 'IN', '464': 'IN',
    '465': 'IN', '466': 'IN', '467': 'IN', '468': 'IN', '469': 'IN',
    '470': 'IN', '471': 'IN', '472': 'IN', '473': 'IN', '474': 'IN',
    '475': 'IN', '476': 'IN', '477': 'IN', '478': 'IN', '479': 'IN',
    '480': 'MI', '481': 'MI', '482': 'MI', '483': 'MI', '484': 'MI',
    '485': 'MI', '486': 'MI', '487': 'MI', '488': 'MI', '489': 'MI',
    '490': 'MI', '491': 'MI', '492': 'MI', '493': 'MI', '494': 'MI',
    '495': 'MI', '496': 'MI', '497': 'MI', '498': 'MI', '499': 'MI',
    '500': 'IA', '501': 'IA', '502': 'IA', '503': 'IA', '504': 'IA',
    '505': 'IA', '506': 'IA', '507': 'IA', '508': 'IA', '509': 'IA',
    '510': 'IA', '511': 'IA', '512': 'IA', '513': 'IA', '514': 'IA',
    '515': 'IA', '516': 'IA',
    '520': 'IA', '521': 'IA', '522': 'IA', '523': 'IA', '524': 'IA',
    '525': 'IA', '526': 'IA', '527': 'IA', '528': 'IA',
    '530': 'WI', '531': 'WI', '532': 'WI', '534': 'WI', '535': 'WI',
    '537': 'WI', '538': 'WI', '539': 'WI', '540': 'WI', '541': 'WI',
    '542': 'WI', '543': 'WI', '544': 'WI', '545': 'WI', '546': 'WI',
    '547': 'WI', '548': 'WI', '549': 'WI',
    '550': 'MN', '551': 'MN', '553': 'MN', '554': 'MN', '555': 'MN',
    '556': 'MN', '557': 'MN', '558': 'MN', '559': 'MN', '560': 'MN',
    '561': 'MN', '562': 'MN', '563': 'MN', '564': 'MN', '565': 'MN',
    '566': 'MN', '567': 'MN',
    '570': 'SD', '571': 'SD', '572': 'SD', '573': 'SD', '574': 'SD',
    '575': 'SD', '576': 'SD', '577': 'SD',
    '580': 'ND', '581': 'ND', '582': 'ND', '583': 'ND', '584': 'ND',
    '585': 'ND', '586': 'ND', '587': 'ND', '588': 'ND',
    '590': 'MT', '591': 'MT', '592': 'MT', '593': 'MT', '594': 'MT',
    '595': 'MT', '596': 'MT', '597': 'MT', '598': 'MT', '599': 'MT',
    '600': 'IL', '601': 'IL', '602': 'IL', '603': 'IL', '604': 'IL',
    '605': 'IL', '606': 'IL', '607': 'IL', '608': 'IL', '609': 'IL',
    '610': 'IL', '611': 'IL', '612': 'IL', '613': 'IL', '614': 'IL',
    '615': 'IL', '616': 'IL', '617': 'IL', '618': 'IL', '619': 'IL',
    '620': 'IL', '622': 'IL', '623': 'IL', '624': 'IL', '625': 'IL',
    '626': 'IL', '627': 'IL', '628': 'IL', '629': 'IL',
    '630': 'MO', '631': 'MO', '633': 'MO', '634': 'MO', '635': 'MO',
    '636': 'MO', '637': 'MO', '638': 'MO', '639': 'MO', '640': 'MO',
    '641': 'MO', '644': 'MO', '645': 'MO', '646': 'MO', '647': 'MO',
    '648': 'MO', '649': 'MO', '650': 'MO', '651': 'MO', '652': 'MO',
    '653': 'MO', '654': 'MO', '655': 'MO', '656': 'MO', '657': 'MO',
    '658': 'MO',
    '660': 'KS', '661': 'KS', '662': 'KS', '664': 'KS', '665': 'KS',
    '666': 'KS', '667': 'KS', '668': 'KS', '669': 'KS', '670': 'KS',
    '671': 'KS', '672': 'KS', '673': 'KS', '674': 'KS', '675': 'KS',
    '676': 'KS', '677': 'KS', '678': 'KS', '679': 'KS',
    '680': 'NE', '681': 'NE', '683': 'NE', '684': 'NE', '685': 'NE',
    '686': 'NE', '687': 'NE', '688': 'NE', '689': 'NE', '690': 'NE',
    '691': 'NE', '692': 'NE', '693': 'NE',
    '700': 'LA', '701': 'LA', '703': 'LA', '704': 'LA', '705': 'LA',
    '706': 'LA', '707': 'LA', '708': 'LA', '710': 'LA', '711': 'LA',
    '712': 'LA', '713': 'LA', '714': 'LA',
    '716': 'AR', '717': 'AR', '718': 'AR', '719': 'AR', '720': 'AR',
    '721': 'AR', '722': 'AR', '723': 'AR', '724': 'AR', '725': 'AR',
    '726': 'AR', '727': 'AR', '728': 'AR', '729': 'AR',
    '730': 'OK', '731': 'OK', '734': 'OK', '735': 'OK', '736': 'OK',
    '737': 'OK', '738': 'OK', '739': 'OK', '740': 'OK', '741': 'OK',
    '743': 'OK', '744': 'OK', '745': 'OK', '746': 'OK', '747': 'OK',
    '748': 'OK', '749': 'OK',
    '750': 'TX', '751': 'TX', '752': 'TX', '753': 'TX', '754': 'TX',
    '755': 'TX', '756': 'TX', '757': 'TX', '758': 'TX', '759': 'TX',
    '760': 'TX', '761': 'TX', '762': 'TX', '763': 'TX', '764': 'TX',
    '765': 'TX', '766': 'TX', '767': 'TX', '768': 'TX', '769': 'TX',
    '770': 'TX', '772': 'TX', '773': 'TX', '774': 'TX', '775': 'TX',
    '776': 'TX', '777': 'TX', '778': 'TX', '779': 'TX', '780': 'TX',
    '781': 'TX', '782': 'TX', '783': 'TX', '784': 'TX', '785': 'TX',
    '786': 'TX', '787': 'TX', '788': 'TX', '789': 'TX', '790': 'TX',
    '791': 'TX', '792': 'TX', '793': 'TX', '794': 'TX', '795': 'TX',
    '796': 'TX', '797': 'TX', '798': 'TX', '799': 'TX',
    '800': 'CO', '801': 'CO', '802': 'CO', '803': 'CO', '804': 'CO',
    '805': 'CO', '806': 'CO', '807': 'CO', '808': 'CO', '809': 'CO',
    '810': 'CO', '811': 'CO', '812': 'CO', '813': 'CO', '814': 'CO',
    '815': 'CO', '816': 'CO',
    '820': 'WY', '821': 'WY', '822': 'WY', '823': 'WY', '824': 'WY',
    '825': 'WY', '826': 'WY', '827': 'WY', '828': 'WY', '829': 'WY',
    '830': 'WY', '831': 'WY',
    '832': 'ID', '833': 'ID', '834': 'ID', '835': 'ID', '836': 'ID',
    '837': 'ID', '838': 'ID',
    '840': 'UT', '841': 'UT', '842': 'UT', '843': 'UT', '844': 'UT',
    '845': 'UT', '846': 'UT', '847': 'UT',
    '850': 'AZ', '852': 'AZ', '853': 'AZ', '855': 'AZ', '856': 'AZ',
    '857': 'AZ', '859': 'AZ', '860': 'AZ', '863': 'AZ', '864': 'AZ',
    '865': 'AZ',
    '870': 'NM', '871': 'NM', '872': 'NM', '873': 'NM', '874': 'NM',
    '875': 'NM', '877': 'NM', '878': 'NM', '879': 'NM', '880': 'NM',
    '881': 'NM', '882': 'NM', '883': 'NM', '884': 'NM',
    '889': 'NV', '890': 'NV', '891': 'NV', '893': 'NV', '894': 'NV',
    '895': 'NV', '897': 'NV', '898': 'NV',
    '900': 'CA', '901': 'CA', '902': 'CA', '903': 'CA', '904': 'CA',
    '905': 'CA', '906': 'CA', '907': 'CA', '908': 'CA', '910': 'CA',
    '911': 'CA', '912': 'CA', '913': 'CA', '914': 'CA', '915': 'CA',
    '916': 'CA', '917': 'CA', '918': 'CA', '919': 'CA', '920': 'CA',
    '921': 'CA', '922': 'CA', '923': 'CA', '924': 'CA', '925': 'CA',
    '926': 'CA', '927': 'CA', '928': 'CA', '930': 'CA', '931': 'CA',
    '932': 'CA', '933': 'CA', '934': 'CA', '935': 'CA', '936': 'CA',
    '937': 'CA', '938': 'CA', '939': 'CA', '940': 'CA', '941': 'CA',
    '942': 'CA', '943': 'CA', '944': 'CA', '945': 'CA', '946': 'CA',
    '947': 'CA', '948': 'CA', '949': 'CA', '950': 'CA', '951': 'CA',
    '952': 'CA', '953': 'CA', '954': 'CA', '955': 'CA', '956': 'CA',
    '957': 'CA', '958': 'CA', '959': 'CA', '960': 'CA', '961': 'CA',
    '967': 'HI', '968': 'HI',
    '970': 'OR', '971': 'OR', '972': 'OR', '973': 'OR', '974': 'OR',
    '975': 'OR', '976': 'OR', '977': 'OR', '978': 'OR', '979': 'OR',
    '980': 'WA', '981': 'WA', '982': 'WA', '983': 'WA', '984': 'WA',
    '985': 'WA', '986': 'WA', '988': 'WA', '989': 'WA', '990': 'WA',
    '991': 'WA', '992': 'WA', '993': 'WA', '994': 'WA',
    '995': 'AK', '996': 'AK', '997': 'AK', '998': 'AK', '999': 'AK'
};

const STATE_NAMES = {
    'AL': 'Alabama', 'AK': 'Alaska', 'AZ': 'Arizona', 'AR': 'Arkansas',
    'CA': 'California', 'CO': 'Colorado', 'CT': 'Connecticut', 'DE': 'Delaware',
    'DC': 'District of Columbia', 'FL': 'Florida', 'GA': 'Georgia', 'HI': 'Hawaii',
    'ID': 'Idaho', 'IL': 'Illinois', 'IN': 'Indiana', 'IA': 'Iowa',
    'KS': 'Kansas', 'KY': 'Kentucky', 'LA': 'Louisiana', 'ME': 'Maine',
    'MD': 'Maryland', 'MA': 'Massachusetts', 'MI': 'Michigan', 'MN': 'Minnesota',
    'MS': 'Mississippi', 'MO': 'Missouri', 'MT': 'Montana', 'NE': 'Nebraska',
    'NV': 'Nevada', 'NH': 'New Hampshire', 'NJ': 'New Jersey', 'NM': 'New Mexico',
    'NY': 'New York', 'NC': 'North Carolina', 'ND': 'North Dakota', 'OH': 'Ohio',
    'OK': 'Oklahoma', 'OR': 'Oregon', 'PA': 'Pennsylvania', 'PR': 'Puerto Rico',
    'RI': 'Rhode Island', 'SC': 'South Carolina', 'SD': 'South Dakota', 'TN': 'Tennessee',
    'TX': 'Texas', 'UT': 'Utah', 'VT': 'Vermont', 'VA': 'Virginia',
    'WA': 'Washington', 'WV': 'West Virginia', 'WI': 'Wisconsin', 'WY': 'Wyoming',
    'VI': 'Virgin Islands'
};
'''
    
    with open(output_path, 'w', encoding='utf-8') as f:
        f.write(js_output)
    
    print(f"Generated {output_path}")

def main():
    script_dir = Path(__file__).parent
    
    # Parse GPCI data
    print("Parsing GPCI data...")
    gpci_data = parse_gpci_data(script_dir / 'GPCI2025.csv')
    state_gpcis = get_state_average_gpci(gpci_data)
    print(f"Found GPCI data for {len(state_gpcis)} states/territories")
    
    # Parse RVU data
    print("Parsing RVU data...")
    rvu_data = parse_rvu_data(script_dir / 'PPRRVU2025_Oct.csv')
    print(f"Found {len(rvu_data)} valid CPT codes")
    
    # Generate state averages
    print("Calculating state averages...")
    cpt_with_states = generate_state_averages(rvu_data, state_gpcis)
    
    # Generate JavaScript file
    output_path = script_dir.parent / 'data' / 'cpt-averages.js'
    generate_javascript(cpt_with_states, output_path)
    
    # Print sample output for verification
    print("\n--- Sample Data (99213) ---")
    if '99213' in cpt_with_states:
        sample = cpt_with_states['99213']
        print(f"Description: {sample['description']}")
        print("Sample state rates:")
        for state in ['NY', 'CA', 'TX', 'FL', 'IL']:
            if state in sample['stateAverages']:
                print(f"  {state}: ${sample['stateAverages'][state]:.2f}")

if __name__ == '__main__':
    main()
